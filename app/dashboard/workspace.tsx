...
export default function Dashboard({email}:{email:string}){
  ...
  const loadArticles=useCallback(async(id:string)=>{
    try {const result=await api<{knowledge:Article[]}>('/api/admin/knowledge?workspaceId='+encodeURIComponent(id));setArticles(result.knowledge);}
    catch(e){setError(e instanceof Error?e.message:'Could not load articles');}
  },[]);
  const loadConversations=useCallback(async(id:string)=>{
    try {const result=await api<{conversations:Conversation[]}>('/api/admin/conversations?workspaceId='+encodeURIComponent(id));setConversations(result.conversations);}
    catch(e){setError(e instanceof Error?e.message:'Could not load inbox');}
  },[]);
  useEffect(()=>{void loadWorkspaces();},[loadWorkspaces]);
  useEffect(()=>{if(!selectedId)return;void loadArticles(selectedId);void loadConversations(selectedId);},[selectedId,loadArticles,loadConversations]);
